$(document).ready(function () {
    ...
    function closeModal(modalType) {
        ...
        if (!$(".edit_modal_box").hasClass("show") && !$(".write_modal_box").hasClass("show")) {
    ...
    async function loadReview() {
        ...
        review.forEach(function (review) {
            ...
            const truncatedContent =
                review.newContent.length > 50 ? review.newContent.substring(0, 50) + "..." : review.newContent;

            // 제목이 너무 길면 자르기
            const truncatedTitle =
                review.newTitle.length > 20 ? review.newTitle.substring(0, 20) + "..." : review.newTitle;
    ...
    $("#review-form").on("submit", async function (event) {
        ...
        try {
            if (file) {
                ...
                const { data: publicUrlData } = supabaseClient.storage.from("review-images").getPublicUrl(fileName);
                filePath = publicUrlData.publicUrl;
            }

            const { data, error } = await supabaseClient.from("NewProductReview").insert([
                {
                    newTitle: title,
                    newContent: content,
    ...
    async function deleteReview(reviewId) {
        ...
        const deleteButton = $(`.modal_buttons .delete-btn[data-post-id="${reviewId}"]`);
        deleteButton.prop("disabled", true).text("삭제 중...");

        try {
            const { error } = await supabaseClient.from("NewProductReview").delete().eq("id", reviewId);
